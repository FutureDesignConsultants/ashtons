...
$current_department = get_post_meta($current_id, '_department', true);
$current_bedrooms   = (int) get_post_meta($current_id, '_bedrooms', true);
$current_price      = (int) get_post_meta($current_id, '_price_actual', true);
$current_location   = get_post_meta($current_id, '_address_three', true);

if (!$current_bedrooms) {
	$current_bedrooms = (int) get_post_meta($current_id, 'bedrooms', true);
}

if (!$current_price) {
	$current_price = (int) get_post_meta($current_id, '_price', true);
}
...
if ($current_price > 0) {
	$min_price = floor($current_price * 0.8);
	$max_price = ceil($current_price * 1.2);
}
...
if ($current_bedrooms > 0) {
	$property_meta_query[] = [
		'key'     => '_bedrooms',
		'value'   => [$current_bedrooms - 1, $current_bedrooms + 1],
		'compare' => 'BETWEEN',
		'type'    => 'NUMERIC',
	];
}
